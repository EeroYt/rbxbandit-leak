...
const actions = {
    upgraderSetFilterSearch({ commit }, value) {
        ...
        actions.upgraderGetItemListSocket(this);
    },
    ...
    upgraderGetItemListSocket({ getters, commit, dispatch }) {
        const data = { page: getters.upgraderItemList.page, search: getters.upgraderFilterSearch, sort: getters.upgraderFilterSort };
        ...
        setTimeout(() => {
            getters.socketUpgrader.emit('sendBet', data, (res) => {
                if(res.success === true) {
                    ...
                    commit('upgrader_set_game', res.games);
                } else {
                    dispatch('notificationShow', res.error);
                }

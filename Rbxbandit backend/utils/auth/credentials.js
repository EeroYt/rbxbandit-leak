...
const authCheckPostCredentialsData = (data) => {
    ...
    } else if(data.loginInfo === undefined || typeof data.loginInfo !== 'string') {
...
const authCheckPostCredentialsRegisterData = (data) => {
    if(data === undefined || data === null) {
        ...
        throw new Error('Your provided password should be at least 4 characters long');
    }
}

const authCheckPostCredentialsRegisterUser = (userDatabase) => {
    if(userDatabase !== null) {
        throw new Error('Your provided email or username is already used.');
    }
}

...
module.exports = () => {
    ...
    router.post('/', rateLimiterStrictMiddleware, async(req, res) => {
        try {
            ...
            // Get sent loginInfo and password
            const loginInfo = req.body.loginInfo.trim();
            ...
            if (settings.general.maintenance.enabled === true && userDatabase.rank !== 'admin') {
                throw new Error('Site is under maintenance. You may not perform this action');
            }
    ...
    router.post('/register', rateLimiterStrictMiddleware, async(req, res) => {
        try {
            ...
            // Get sent username, email and password
            let username = req.body.username === undefined ? null : req.body.username.trim();
            ...
            // Set random username if not provided with one
            username = username === null ? `User#${crypto.randomInt(100, 10000000)}` : username;

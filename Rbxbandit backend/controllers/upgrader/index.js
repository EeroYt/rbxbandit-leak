...
const upgraderSendBetSocket = async(io, socket, user, data, callback) => {
    try {
        ...
        callback({ success: true, user: { ...dataDatabase[0], balance: Math.floor((user.balance + amountPayout) - amount) }, games: dataDatabase[3] });
        ...
        setTimeout(async() => {
            ...
            generalAddBetsList(io, { ...dataDatabase[3], user: generalUserGetFormated(user), method: 'upgrader' });
        }, 1000 * 5);
